...
static int SKIN_spinbox(const WIDGET_ITEM_DRAW_INFO * pDrawItemInfo, unsigned char C)
{
	...
	//case WIDGET_ITEM_DRAW_FRAME:
		break;
	default: return SPINBOX_DrawSkinFlex(pDrawItemInfo);//emWin默认控件绘制函数
	...
	case WIDGET_ITEM_DRAW_BUTTON_R:
		...
		SPINBOX_SetUserClip(pDrawItemInfo);//设置1个用户剪切区
		...
		else if(pDrawItemInfo->ItemIndex == SPINBOX_SKINFLEX_PI_FOCUSSED)
