...
DECLARE_FONT(PROP_GL);

extern const tGUI_ENC_APIList GUI_ENC_APIList_GB2312;
extern const tGUI_ENC_APIList GUI_ENC_APIList_GBK;
extern const tGUI_ENC_APIList GUI_ENC_APIList_BIG5;
extern const tGUI_ENC_APIList GUI_ENC_APIList_UTF8;

typedef struct GUI_FONT_PROP_GL {
  U16P First;                                           //First character
  U16P Last;                                            //Last character
  const unsigned char GUI_UNI_PTR * pXsize;             //Address of first character
  const struct GUI_FONT_PROP_GL GUI_UNI_PTR *pNext;     //Pointer to next
} GUI_FONT_PROP_GL;

#define GUI_FONTTYPE_PROP_GL   \
  GUIPROP_GL_DispChar,         \
  GUIPROP_GL_GetCharDistX,     \
  GUIPROP_GL_GetFontInfo,      \
  GUIPROP_GL_IsInFont,         \
  (GUI_GETCHARINFO *)0,        \
  &GUI_ENC_APIList_GBK
//&GUI_ENC_APIList_GB2312
//&GUI_ENC_APIList_GBK
//&GUI_ENC_APIList_BIG5
//&GUI_ENC_APIList_UTF8


typedef struct GUI_FONT_GL {
  GUI_DISPCHAR     * pfDispChar; 
  GUI_GETCHARDISTX * pfGetCharDistX; 
  GUI_GETFONTINFO  * pfGetFontInfo; 
  GUI_ISINFONT     * pfIsInFont;
  ...
  U8 LHeight;                       //Height of a small lower case character (a,x)
  U8 CHeight;                       //Height of a small upper case character (A,X)
  U8 Encode;                        //1:gb2312 2:gbk 4:big5 8:unicode(monospaced) 16:unicode(non-monospaced)
  const U8  GUI_UNI_PTR * pFont;    //Library of [ascii]:[gb2312/gbk/big5/unicode/utf-8]
} GUI_FONT_GL;
