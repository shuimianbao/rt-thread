...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    GUI_USE_PARA(PixelIndex);
    {
      ...
      //
		GLCD_WPXI(y, x);
		GLCD_WDAT(PixelIndex>>13, PixelIndex>>1);
    }
...
*       _FillRect
*/
static const/*speed?*/ unsigned int tabMULT[32] = {
0xfffffffc/*31*/,0xfffffffc/*30*/,0xfffffffc/*29*/,0xfffffffc/*28*/,0xfffffffc/*27*/,0xfffffffc/*26*/,0xfffffffc/*25*/,0xfffffffc/*24*/,
0xfffffffc/*23*/,0xfffffffc/*22*/,0xfffffffc/*21*/,0xfffffffc/*20*/,0xfffffffc/*19*/,0xfffffffc/*18*/,0xfffffffc/*17*/,0xfffffffc/*16*/,
0xfffffff8/*15*/,0xfffffff8/*14*/,0xfffffff8/*13*/,0xfffffff8/*12*/,0xfffffff8/*11*/,0xfffffff0/*10*/,0xfffffff0/*09*/,0xfffffff0/*08*/,
0xffffffe0/*07*/,0xffffffc0/*06*/,0xffffff80/*05*/,0xffffff00/*04*/,0xfffff800/*03*/,0xffff0000/*02*/,0x00000000/*01*/,0x00000000/*00*/};
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
	...
	DrawMode = GUI_pContext->DrawMode;

	
	if((x=x1-x0)<0) return; if((y=y1-y0)<0) return;
	if((x>=31) || (y>=31)) goto GRAPH;
	else if((tabMULT[x+1]>>(y+1))&1) goto GRAPH;	//(x+1)*(y+1)>=32->1 (x+1)*(y+1)<32->0

	if(DrawMode & LCD_DRAWMODE_XOR)
	{
		if(x >= y) for(; y0 <= y1; y0++) {GLCD_WPXI(y0, x0); for(x=x0; x<=x1; x++) GLCD_WXOR();}
		else for(; x0 <= x1; x0++) {GLCD_WPYI(y0, x0); for(y=y0; y <= y1; y++) GLCD_WXOR();}
	}
	else
	{
		if(x >= y) for(; y0 <= y1; y0++) {GLCD_WPXI(y0, x0); for(x=x0; x<=x1; x++) GLCD_WDAT(ColorIndex>>13, ColorIndex>>1);}
		else for(; x0 <= x1; x0++) {GLCD_WPYI(y0, x0); for(y=y0; y <= y1; y++) GLCD_WDAT(ColorIndex>>13, ColorIndex>>1);}
	}
	return;
...
static void _DrawHLine(GUI_DEVICE * pDevice, int x0, int y, int x1) {
	...
	if((x1-x0) >= 31) goto GRAPH;

	if (DrawMode & LCD_DRAWMODE_XOR)
	{
		GLCD_WPXI(y, x0);
		for(; x0 <= x1; x0++) GLCD_WXOR();
	}
	else
	{
		GLCD_WPXI(y, x0);
		for(; x0 <= x1; x0++) GLCD_WDAT(ColorIndex>>13, ColorIndex>>1);
	}
	return;
...
static void _DrawVLine(GUI_DEVICE * pDevice, int x, int y0, int y1) {
	...
	if((y1-y0) >= 31) goto GRAPH;
	
	if(DrawMode & LCD_DRAWMODE_XOR)
	{
		GLCD_WPYI(y0, x);
		for (; y0 <= y1; y0++) GLCD_WXOR();
	}
	else
	{
		GLCD_WPYI(y0, x);
		for (; y0 <= y1; y0++) GLCD_WDAT(ColorIndex>>13, ColorIndex>>1);
	}
	return;
